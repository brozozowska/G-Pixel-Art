...
func block(x: Int, y: Int, width: Int, height: Int, color: Color) {
    for h in 0 ... height - 1 {
        hline(x: x, y: y - h, length: width, color: color)
    }
}

block(x: 1, y: 38, width: 38, height: 38, color: .blue)
/*:
 ### Fill Speed
...
func newBlock(x: Int, y: Int, width: Int, height: Int, color: Color) {
    for x in x ... x + width - 1 {
        for y in y ... y + height - 1 {
            display.setPixel(x: x, y: y, color: color)
            pixelsArray.append(Pixel(x: x, y: y, color: color))
        }
    }
}
newBlock(x: 2, y: 2, width: 36, height: 36, color: .red)
