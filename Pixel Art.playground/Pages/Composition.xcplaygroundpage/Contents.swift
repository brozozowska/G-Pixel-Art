...
func legs() {
    display.batchSetPixels(block(x: 18 + 5, y: 10 + 5, width: 2, height: 4, color: .blue))
}
func torso() {
    display.batchSetPixels(block(x: 17 + 5, y: 14 + 5, width: 4, height: 3, color: .white))
}
func head() {
    display.batchSetPixels(block(x: 18 + 5, y: 17 + 5, width: 2, height: 2, color: .yellow))
}
